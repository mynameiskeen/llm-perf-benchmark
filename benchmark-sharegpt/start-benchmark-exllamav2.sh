...
do
  ...
  /data/miniconda3/envs/vllm/bin/python benchmark_serving.py --backend openai-chat \
      --tokenizer $MODEL_PATH4 \
      --endpoint /v1/chat/completions \
      --model Llama3-8B-Instruct-exl2-Int4 --dataset-name sharegpt \
      --dataset-path ./ShareGPT_V3_unfiltered_cleaned_split.json \
      --num-prompts 500 --port 8010 --save-result \
      --result-dir result_outputs/ \
      --result-filename exllamav2_llama3-8b-instruct-awq_qps_$rate.json \
      --request-rate $rate
  if [ $? -ne 0 ]; then

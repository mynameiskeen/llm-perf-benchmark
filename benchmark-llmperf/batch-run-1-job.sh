...
do
  ...
  start_secs=$(date +%s)
  echo "$(date +"%Y-%m-%d_%H-%M-%S"): Starting scenario: $scn, job 1 - concurrency: $con ..."
  ./start-benchmark.sh $scn $con $MODEL_NAME 1 $timestamp >> $BATCH_RESULT
  if [ $? -ne 0 ]; then
    echo_error "$(date +"%Y-%m-%d_%H-%M-%S"): Benchmark job 1 failed, please check log file logs/${timestamp}.${scn}-${con}-${MODEL_NAME}-${i}.out"
  fi
  end_secs=$(date +%s)
  duration=$(( $end_secs - start_secs ))
  echo "$(date +"%Y-%m-%d_%H-%M-%S"): Scenarios: $scn, job 1 finished successfully, elapsed time: $duration seconds."
done
echo "$(date +"%Y-%m-%d_%H-%M-%S"): All jobs have been done, please check $BATCH_RESULT for batch results."